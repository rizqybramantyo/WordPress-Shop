...
abstract class table {
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObj('field', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return db::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function store($data, $method = 'INSERT', $where = '') {
        ...
        if(empty($fields)) {
            $this->_addError(lang::_('Nothig to update'));
            return false;
        }
        ...
        if(db::query($query)) {
			...
			$this->_addError(lang::_('Database error. Please contact your developer.'));
        return false;
    }
            ...
            foreach($data as $k => $v) {
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if($this->_fields[$k]->adapt['dbTo']) 
                        $val = fieldAdapter::_($val, $this->_fields[$k]->adapt['dbTo'], fieldAdapter::DB);
                    if($validate) {
                        if (is_object($this->_fields[$k])) {
                            $objForValidation = clone $this->_fields[$k];
                            $objForValidation->setValue($val);
                            if($errors = validator::_($objForValidation)) {
    ...
    /**
     * Add new field for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObj('field', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return db::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(S_DATE_FORMAT, db::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return db::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }

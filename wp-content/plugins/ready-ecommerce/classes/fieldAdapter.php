...
class fieldAdapter {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapter', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['options'] = array();
    ...
    static public function taxDataHtml($field) {
        ...
        return '<div>'. lang::_('Apply To'). '
            <div id="tax_address">
                <b>'. lang::_('Address'). '</b><br />
                '. lang::_('Destination'). ':'. html::checkboxlist('params[dest]', array('options' => $listOfDest)). '<br />
                '. lang::_('Country'). ':'. html::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. lang::_('Categories'). '</b><br />
                '. (empty($cOptions) ? lang::_('You have no categories') : html::checkboxlist('params[categories][]', array('options' => $cOptions))). '<br />
                    <b>'. lang::_('Brands'). '</b><br />
                '. (empty($bOptions) ? lang::_('You have no brands') : html::checkboxlist('params[brands][]', array('options' => $bOptions))). '<br />
            </div>
            <div>'. lang::_('Tax Rate').': '. html::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. lang::_('Absolute').': '. html::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? lang::_('Not selected') : lang::_($notSelected);
			}
    ...
    static public function getStates($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if($notSelected) {
				...
				$options[ $notSelected ][0] = array('name' => lang::_( $notSelectedLabel ), 'country_id' => NULL);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frame::_()->getModule('options')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= lang::_($key).':<br />';
                $output .= html::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= lang::_('class').':<br />';
                $output .= html::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= lang::_('id').':<br />';
                $output .= html::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['options']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['options'][0] = in_array(0,$field->value['categories'])?lang::_('Deselect All'):lang::_('Select All');
            foreach($categories as $c) {
                $field->htmlParams['options'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frame::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frame::_()->getTable('states')
				->leftJoin( frame::_()->getTable('countries'), 'country_id' )

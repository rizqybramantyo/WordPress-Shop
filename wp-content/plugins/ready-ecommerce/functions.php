...
if(!function_exists('importClass')) {
    function importClass($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path)
                $path = S_CLASSES_DIR. $class. '.php';
            return import($path);
           ...
           /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, S_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], S_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassName')) {
    function toeGetClassName($class) {
        $className = '';
        if(class_exists(S_CLASS_PREFIX. $class)) 
            $className = S_CLASS_PREFIX. $class;
        else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObj')) {
    function toeCreateObj($class, $params) {
        $className = toeGetClassName($class);
        $obj = NULL;
 ...
 * @return array
 */
if(!function_exists('prepareParams')) {
	function prepareParams(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utils::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCode($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCode')) {
	function prepareFieldCode($string) {   
